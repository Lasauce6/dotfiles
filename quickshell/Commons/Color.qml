	...
	FileView {
		...
		onFileChanged: {
			...
			try {
				...
				Logger.error("Color", "Error reloading colors:", error.toString())
			}
		}
		onAdapterUpdated: {
			...
			try {
				...
				Logger.error("Color", "Error writing colors:", error.toString())
			}
		}
		onLoadFailed: function (error) {
			if (error.toString().includes("No such file") || error === 2) {
				...
				try {
					writeAdapter()
					Logger.log("Color", "Created default colors.json")
				} catch (writeError) {
					Logger.error("Color", "Failed to create colors.json:", writeError.toString())
					Logger.log("Color", "Using built-in default colors")
				}
			} else {
				Logger.warn("Color", "Failed to load colors.json:", error.toString())
				Logger.log("Color", "Using built-in default colors")
			}
		}
		JsonAdapter {
